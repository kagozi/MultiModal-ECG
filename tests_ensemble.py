...
from models import (
    ...
    HybridSwinTransformerECG, HybridSwinTransformerEarlyFusion,
    HybridSwinTransformerLateFusion,
    EfficientNetEarlyFusion,
    EfficientNetFusionECG,
    ResNet50EarlyFusion,
    ResNet50LateFusion,
    ResNet50ECG
)
from benchmark import (
    XResNet1d101, load_ptbxl_dataset, aggregate_diagnostic_labels,
    preprocess_signals, prepare_labels
)
...
class ECGDataset(Dataset):
    ...
    def __init__(self, X, y):
        # (N, time, channels) -> (N, channels, time)
...
def save_timing_reports(timing_results, save_dir):
    """
    Save timing summary (JSON + CSV).
    """
    timing_json_path = os.path.join(save_dir, "inference_timing_summary.json")
    with open(timing_json_path, "w") as f:
        json.dump(timing_results, f, indent=2)
    print(f"Saved timing JSON: {timing_json_path}")

    timing_csv_path = os.path.join(save_dir, "inference_timing_summary.csv")
    with open(timing_csv_path, "w") as f:
        f.write("model,total_s,n_samples,ms_per_sample,samples_per_s,batch_size,num_workers,device\n")
        for name, t in timing_results.items():
            f.write(
                f"{name},{t.get('total_s', '')},{t.get('n_samples', '')},{t.get('ms_per_sample', '')},"
                f"{t.get('samples_per_s', '')},{t.get('batch_size', '')},{t.get('num_workers', '')},{t.get('device', '')}\n"
            )
    print(f"Saved timing CSV: {timing_csv_path}")


def estimate_ensemble_cost(selected_names, timing_results, n_samples, label="ensemble"):
    """
    Ensemble compute cost (inference) is approximately the sum of forward-pass costs
    of all constituent models + negligible aggregation overhead.
    """
    total_s = 0.0
    missing = []
    for n in selected_names:
        if n in timing_results and "total_s" in timing_results[n]:
            total_s += float(timing_results[n]["total_s"])
        else:
            missing.append(n)
    ...
    stats = {
        ...
        "n_samples": int(n_samples),
        "ms_per_sample": float((total_s / max(n_samples, 1)) * 1000.0),
        "samples_per_s": float(n_samples / max(total_s, 1e-12)) if total_s > 0 else 0.0,
        "note": "Estimated as sum of constituent model inference times (aggregation overhead ignored).",
...
def get_predictions(model, dataloader, config):
    ...
    mode = config['mode']
    is_dual = (config.get('model') == 'DualStream') or (mode == 'both')
...
def evaluate_all_models(metadata, X_test_raw, y_test):
    ...
    Also records per-model inference timing over the full test set.
    """
    ...
    if os.path.exists(baseline_checkpoint):
        ...
        if load_model_checkpoint_safely(model, baseline_checkpoint, DEVICE):
            ...
            # ---- Timing: baseline inference ----
            timing = time_inference(model, test_loader_raw, baseline_config)
            timing_results["ResNet1D-Baseline"] = timing
            print(f"[Timing] ResNet1D-Baseline: {timing['total_s']:.3f}s | "
                  f"{timing['ms_per_sample']:.3f} ms/sample | {timing['samples_per_s']:.2f} samples/s")
    ...
    for result_file in result_files:
        ...
        try:
            ...
            timing = time_inference(model, test_loader, config)
            timing_results[model_name] = timing
            print(f"[Timing] {model_name}: {timing['total_s']:.3f}s | "
                  f"{timing['ms_per_sample']:.3f} ms/sample | {timing['samples_per_s']:.2f} samples/s")
    ...
    # Save per-model timing report once at the end (not inside the loop)
    save_timing_reports(timing_results, ENSEMBLE_PATH)

    return all_model_results, y_true, timing_results
...
def create_ensemble(model_results, y_true, metadata, timing_results, method='average', top_k=None):
    """
    Creates ensembles using stored logits (y_scores) and also estimates compute cost
    as the sum of constituent model inference times.
    """
    ...
    if top_k:
        ...
        print(f"Top {top_k} models: {selected_names}")
    else:
    ...
    if method == 'average':
        ...
        weights = np.array([model_results[n]['metrics']['macro_auc'] for n in selected_names], dtype=float)
        weights /= weights.sum()
    ...
    # ---- Ensemble compute cost (estimated) ----
    ens_cost = estimate_ensemble_cost(
        selected_names=selected_names,
        timing_results=timing_results,
        n_samples=len(y_true),
        label=ensemble_name
    )
    print(f"[Ensemble Timing ~] {ensemble_name}: {ens_cost['total_s']:.3f}s | "
          f"{ens_cost['ms_per_sample']:.3f} ms/sample | {ens_cost['samples_per_s']:.2f} samples/s")
    ...
    # Attach timing into returned metrics bundle (so it can be saved)
    metrics_out = dict(metrics)
    metrics_out["timing_estimate"] = ens_cost
    metrics_out["members"] = selected_names

    return metrics_out, ensemble_name
...
def main():
    ...
    model_results, y_true, timing_results = evaluate_all_models(metadata, X_test, y_test)
    ...
    ensemble_results = []
    metrics, name = create_ensemble(model_results, y_true, metadata, timing_results, method='average')
    ensemble_results.append((name, metrics))
    metrics, name = create_ensemble(model_results, y_true, metadata, timing_results, method='weighted')
    ensemble_results.append((name, metrics))

    if len(model_results) >= 3:
        metrics, name = create_ensemble(model_results, y_true, metadata, timing_results, method='average', top_k=3)
        ensemble_results.append((name, metrics))
        metrics, name = create_ensemble(model_results, y_true, metadata, timing_results, method='weighted', top_k=3)
        ensemble_results.append((name, metrics))

    if len(model_results) >= 5:
        metrics, name = create_ensemble(model_results, y_true, metadata, timing_results, method='average', top_k=5)
        ensemble_results.append((name, metrics))
        metrics, name = create_ensemble(model_results, y_true, metadata, timing_results, method='weighted', top_k=5)
        ensemble_results.append((name, metrics))

    plot_model_comparison(model_results, ensemble_results, metadata)

    # Save summary (metrics + ensemble timing estimates)
    summary = {
        'individual_models': {
            n: {k: v for k, v in r['metrics'].items() if k in ['macro_auc', 'f1_macro', 'f_beta_macro']}
            for n, r in model_results.items()
        },
        'individual_timing': timing_results,
        'ensembles': {
            n: {k: v for k, v in m.items() if k in ['macro_auc', 'f1_macro', 'f_beta_macro', 'timing_estimate', 'members']}
    ...
    print("FINAL SUMMARY")
    print("=" * 80)
    print(f"{'Model':<40} | {'AUC':<8} | {'F1':<8} | {'F-beta':<8} | {'ms/sample':<10}")
    print("-" * 95)

    # Per-model print with timing
    for name, res in sorted(model_results.items(), key=lambda x: x[1]['metrics']['macro_auc'], reverse=True):
        m = res['metrics']
        t = timing_results.get(name, {})
        msps = t.get("ms_per_sample", float("nan"))
        print(f"{name:<40} | {m['macro_auc']:.4f}   | {m['f1_macro']:.4f}   | {m['f_beta_macro']:.4f}   | {msps:>8.3f}")

    print("-" * 95)

    # Ensembles print with estimated timing
    for name, m in ensemble_results:
        t = m.get("timing_estimate", {})
        msps = t.get("ms_per_sample", float("nan"))
        print(f"{name:<40} | {m['macro_auc']:.4f}   | {m['f1_macro']:.4f}   | {m['f_beta_macro']:.4f}   | {msps:>8.3f}")
