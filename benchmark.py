# benchmark.py
# ============================================================================
# XRESNET1D BENCHMARK REPLICATION - PURE PYTORCH (WITH TRAINING TIME ESTIMATES)
# ============================================================================
...
class XResNet1d101(nn.Module):
    ...
    def __init__(self, input_channels=12, num_classes=5, base_filters=64, stem_ks=7, block_ks=3, drop_rate=0.0):
        ...
        self.stem = nn.Sequential(
            nn.Conv1d(input_channels, base_filters, kernel_size=stem_ks, stride=2, padding=stem_ks // 2, bias=False),
            nn.BatchNorm1d(base_filters),
            nn.ReLU(inplace=True),
            nn.MaxPool1d(kernel_size=3, stride=2, padding=1)
        )

        # ResNet-101 style depth: [3, 4, 23, 3]
        ...
        self.fc = nn.Linear(base_filters * 8 * 2, num_classes)  # *2 concat pooling
...
def compute_metrics(y_true, y_pred, y_scores):
    ...
    return {'macro_auc': macro_auc, 'f_beta_macro': f_beta}
...
def plot_confusion_matrix_all_classes(y_true, y_pred, class_names, save_path=None, title="Confusion Matrix - All Classes"):
    ...
    sns.heatmap(
        cm_combined, annot=True, fmt=".0f", cmap="Blues",
...
# ============================================================================
# STEP 8: MAIN TRAINING PIPELINE (WITH TIMING)
# ============================================================================
...
def main():
    ...
    print("=" * 80)
    print("XRESNET1D101 BENCHMARK REPLICATION (WITH TRAINING TIME ESTIMATES)")
    print("=" * 80)
    ...
    # ---- Timing log metadata ----
    timing_log = {
        "run_name": "XResNet1d101_raw",
        "gpu": get_gpu_info(),
        "device": str(DEVICE),
        "sampling_rate": SAMPLING_RATE,
        "batch_size": BATCH_SIZE,
        "num_workers": 4,
        "epochs_probe": PROBE_EPOCHS,
        "epochs_full": EPOCHS,
    ...
    print("\n[Timing] Starting linear probing timing...")
    _sync_cuda()
    ...
    timing_log["probe_total_s"] = float(probe_s)
    timing_log["probe_total_h"] = float(probe_s / 3600.0)
    timing_log["probe_avg_epoch_s"] = float(probe_s / max(PROBE_EPOCHS, 1))
    print(f"[Timing] Probe total: {fmt_seconds(probe_s)} ({probe_s/3600:.3f} h)")
    ...
    print("\n[Timing] Starting full fine-tune timing...")
    _sync_cuda()
    ...
    timing_log["full_total_s"] = float(full_s)
    timing_log["full_total_h"] = float(full_s / 3600.0)
    timing_log["full_avg_epoch_s"] = float(full_s / max(EPOCHS, 1))
    timing_log["best_val_auc"] = float(best_val_auc)
    print(f"[Timing] Full total: {fmt_seconds(full_s)} ({full_s/3600:.3f} h)")

    # Total training time (probe + full)
    train_s = probe_s + full_s
    timing_log["train_total_s"] = float(train_s)
    timing_log["train_total_h"] = float(train_s / 3600.0)
    print(f"[Timing] Total training time: {fmt_seconds(train_s)} ({train_s/3600:.3f} h)")

    # Step 8: Test
    print("\n[8/8] Testing on test set...")

    # Optional: time test evaluation separately
    print("\n[Timing] Starting test evaluation timing...")
    _sync_cuda()
    ...
    _sync_cuda()
    test_s = time.perf_counter() - t_test0
    timing_log["test_eval_s"] = float(test_s)
    timing_log["test_eval_h"] = float(test_s / 3600.0)
    print(f"[Timing] Test evaluation time: {fmt_seconds(test_s)} ({test_s/3600:.4f} h)")

    timing_log["test_macro_auc"] = float(test_metrics["macro_auc"])
    timing_log["test_fbeta_macro"] = float(test_metrics["f_beta_macro"])
    ...
    plot_confusion_matrix_all_classes(
        test_labels, test_pred_binary, mlb.classes_,
        save_path=os.path.join(RESULTS_PATH, 'resnet_1d_confusion_matrix_combined.png'),
        title="Confusion Matrix - All Classes"
    )

    # Save timing log (JSON)
    timing_path = os.path.join(RESULTS_PATH, "xresnet1d101_training_timing.json")
    with open(timing_path, "w") as f:
        json.dump(timing_log, f, indent=2)
    print(f"\n[Timing] Saved timing log: {timing_path}")
